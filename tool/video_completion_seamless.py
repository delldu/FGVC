...
def complete_flow(args, corrFlow, flow_mask, mode, edge=None):
    ...
    for i in range(nFrame):
        ...
        utils.frame_utils.writeFlow(
            os.path.join(args.outroot, "flow_comp", mode + "_flo", "%05d.flo" % i),
            compFlow[:, :, :, i],
        )
...
def video_completion_seamless(args):
    ...
    if args.mode == "video_extrapolation":
        ...
        (
            ...
            start_point,
            end_point,
        ) = extrapolation(args, video, corrFlowF, corrFlowB)
    ...
    while np.sum(mask) > 0:
        ...
        for indFrame in range(nFrame):
            ...
            gradient_x_filled[:, :, :, indFrame] = np.concatenate(
                (
                    np.diff(video_comp[:, :, :, indFrame], axis=1),
                    np.zeros((imgH, 1, 3), dtype=np.float32),
                ),
                axis=1,
            )
            gradient_y_filled[:, :, :, indFrame] = np.concatenate(
                (
                    np.diff(video_comp[:, :, :, indFrame], axis=0),
                    np.zeros((1, imgW, 3), dtype=np.float32),
                ),
                axis=0,
            )
    ...
    for i in range(nFrame):
        ...
        cv2.imwrite(
            os.path.join(args.outroot, "frame_seamless_comp_" + "final", "%05d.png" % i),
            img,
        )
        imageio.mimwrite(
            os.path.join(args.outroot, "frame_seamless_comp_" + "final", "final.mp4"),
            video_comp_,
            fps=12,
            quality=8,
            macro_block_size=1,
