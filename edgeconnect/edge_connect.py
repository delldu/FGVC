            ...
            for items in train_loader:
                ...
                # inpaint model
                    ...
                    (
                        ...
                        dis_loss,
                        logs,
                    ) = self.inpaint_model.process(images, images_filled, edges, masks)
                ...
                # inpaint with edge model
                    ...
                    (
                        ...
                        dis_loss,
                        logs,
                    ) = self.inpaint_model.process(images, images_filled, outputs.detach(), masks)
                    ...
                    if self.config.FLO == 0:
                        ...
                        psnr = self.psnr(
                            torch.from_numpy(self.flow2img(images)),
                            torch.from_numpy(self.flow2img(outputs_merged)),
                        )
                    else:
                        assert 0
                ...
                # joint model
                    ...
                    (
                        ...
                        e_dis_loss,
                        e_logs,
                    ) = self.edge_model.process(images_gray, edges, masks)
                    ...
                    (
                        ...
                        i_dis_loss,
                        i_logs,
                    ) = self.inpaint_model.process(images, images_filled, e_outputs, masks)
        ...
        for items in val_loader:
            ...
            # inpaint with edge model
                ...
                outputs, gen_loss, dis_loss, logs = self.inpaint_model.process(
                    images, images_filled, outputs.detach(), masks
                )
    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            if self.debug:
                ...
                imsave(
                    masked,
                    os.path.join(self.results_path, fname + "_masked." + fext),
                )
        ...
        for idx in range(len(flows)):
            imgs = np.concatenate(
                (
                    imgs,
                    np.expand_dims(
                        (self.flow_to_image(flows[idx, :, :, :].cpu().numpy(), global_max)),
                        0,
                    ),
                ),
                axis=0,
